# Product Overview — knative-agents-fullstack-e2e
...
8. **Per-feature KnativeAgent reconciliation**: the
...
4. Deploy SPIRE (server + agent), the operator, and a fake gateway
   (echo TCP + echo HTTP).
5. Apply a sample CR chain (Platform + KnativeAgent + ModelProvider +
...
3. CloudWatch Events rule on instances tagged
   `knative-agents-e2e=L2` older than 1 hour → Lambda → terminate.
