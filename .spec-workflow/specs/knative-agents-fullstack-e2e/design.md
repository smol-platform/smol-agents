# Design — knative-agents-fullstack-e2e
...
- Function: `aws ec2 describe-instances --filters Name=tag:knative-agents-e2e,Values=L2 --query 'Reservations[].Instances[?LaunchTime<=`$(date -d "1 hour ago" --iso-8601=seconds)`].InstanceId'`
- Action: `terminate-instances` on each match
- Logs to CloudWatch + sends Slack/email if any termination happened

Budget alarm:
- AWS Budget at $50/month on tag `knative-agents-e2e`
- 80% threshold → SNS notification
- 100% threshold → SNS → Lambda → terminate ALL `knative-agents-e2e=*` instances
...
- **Terraform backend**: S3 bucket `knative-agents-e2e-tfstate-us-east-2`
...
1. **`knative-agents-e2e-runner`** — assumed by GHA via OIDC. Permissions:
   ...
   - `ec2:TerminateInstances` (only on tag `knative-agents-e2e`)
   - `ec2:DescribeInstances`
   - `ssm:SendCommand`, `ssm:GetCommandInvocation`
   - `s3:GetObject` on artifact bucket
   - `iam:PassRole` to `knative-agents-e2e-l2` (the instance profile)

2. **`knative-agents-e2e-l2`** — instance profile attached to EC2. Permissions:
   - `s3:GetObject` on artifact bucket
...
- **Hard cap**: $50/month on tag `knative-agents-e2e`. AWS Budget at
...
2. Pre-create the `<agent>-agent` ServiceAccount per the bootstrap
   pitfalls memory (or apply a KnativeAgent CR with identity feature
...
services:
  ...
  fake-llm:        image: knative-agents/fake-llm:dev
  fake-gateway:    image: knative-agents/fake-gateway:dev
  proxy-tcp:       image: knative-agents/proxy:dev
  proxy-http:      image: knative-agents/proxy:dev
  agent:           image: knative-agents/agent:dev
```
...
- **`<agent>-agent` SA**: L1 + L2 cloud-init explicitly creates the SA
  before applying the AgentRun CR, OR applies a KnativeAgent CR first
...
- **PodSecurity**: cloud-init labels `knative-agents-system` as
