# Tasks — smol-agents-fullstack-e2e
...
| 4 — L2 cloud-init | ✅ done | scripts/aws-l2/cloud-init.yaml.tmpl with k0s+Kata+SPIRE+operator+samples; READY/FAILED sentinels gate on actual pod health (cert-manager + spire + operator + CRDs); spot-interruption log-upload watcher |
| 5 — L2 driver | ✅ done | Provision/Teardown + l2Env via SSM SendCommand drives `shared.RunAll(t, env, shared.All())` against the bootstrapped k0s cluster. Full scenario set runs unchanged at L2 (wgClient skips cleanly per Caps gate). Mock-SSM unit tests cover Apply/Exec/runSSM polling without AWS. `make e2e-l2-smoke` provides a cheap (~$0.10) Provision+sentinel+Teardown variant for cloud-init drift detection. |
| 6 — Hardening | ✅ done | coverage gate enforces 39/39 R-E2E-* IDs mapped |

Across all rings: **10 scenarios passing for real** (3 at L0, 7 at
L1, all carrying through to L2 via the same shared API). All five
phases scaffolded and integration-ready; remaining work is the
deferred eBPF-in-kind cross-arch BPF compilation and AgentRun
real-Pod execution at L1 (both require net-new operator+agent code,
not test-infrastructure work).
...
- [ ] **T-2.2** — Pre-flight setup: label `smol-agents-system` ns
...
- [ ] **T-2.3** — Apply SmolAgent CR with identity feature enabled
...
- [ ] **T-3.1** — `infra/terraform/aws-e2e/` Terraform module:
  ...
  - Security group: 22 (SSH disabled, kept for break-glass), 80, 443
    (egress only)
  - IAM role `smol-agents-e2e-l2` (instance profile) with S3 GetObject
    + SSM agent registration
  - IAM role `smol-agents-e2e-runner` (assumed by GHA OIDC) with
    EC2 RunInstances/TerminateInstances/DescribeInstances + SSM SendCommand +
    PassRole + ECR pull
  - S3 bucket `smol-agents-e2e-artifacts-us-east-2` for cloud-init
    payloads and test logs (lifecycle: 7-day expiration)
  - ECR repos: operator, agent, ebpf-loader, secret-proxy, fake-llm,
    fake-gateway
  - Backend: S3 `smol-agents-e2e-tfstate-us-east-2` + DynamoDB lock
  - Tags every resource with `smol-agents-e2e=infra`
...
- [ ] **T-3.2** — Sweeper Lambda (`infra/terraform/aws-e2e/sweeper/`):
  ...
  tagged `smol-agents-e2e=L2` whose LaunchTime > 1h ago, terminates
...
- [ ] **T-3.3** — Budget alarm + nuke Lambda
  ...
  `smol-agents-e2e`, alarms at 50/80/100%. 100% triggers SNS →
  Lambda → terminate ALL `smol-agents-e2e=*` tagged instances.
...
- [ ] **T-5.6** — `make e2e-clean-aws`: sweeps every
  `smol-agents-e2e=*` tagged resource. Manual escape hatch when
