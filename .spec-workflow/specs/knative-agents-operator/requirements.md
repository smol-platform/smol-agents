# Requirements — knative-agents-operator
...
#### R-OP-API-2 — `KnativeAgentPlatform` cluster CR
...
**Acceptance Criteria:**
1. EXACTLY ONE `KnativeAgentPlatform` SHALL be permitted per cluster
   (validating webhook enforces).
2. WHEN the `KnativeAgentPlatform` does not exist THEN every
   `KnativeAgent` SHALL stay in `Pending` with reason
   `PlatformAbsent`.
...
1. EVERY field in the CR types SHALL have a `// +kubebuilder:`
   ...
   by the API server.
2. `kubectl explain knativeagent.spec.features.identity` SHALL
...
**Acceptance Criteria:**
1. `KnativeAgentPlatform.spec.featurePolicy` SHALL accept a list of
   `{feature: <name>, allowed: <bool>, defaultEnabled: <bool>}`
   entries.
2. WHEN `allowed: false` AND a `KnativeAgent` enables that feature
...
1. EVERY resource the operator creates SHALL set
   `metadata.ownerReferences` to the parent `KnativeAgent` (or
   `KnativeAgentPlatform`) with `controller: true`.
...
1. THE validating webhook SHALL reject:
   ...
   - `mode: insecure` without `annotations.knative-agents.stigen.ai/allow-insecure: "true"`
   - `runtimeClass: runc` without `sandbox.allowHostEscape: true`
   - feature combinations forbidden by `KnativeAgentPlatform.spec.featurePolicy`
...
1. WHEN a field is unset THE mutating webhook SHALL fill in the
   default from `KnativeAgentPlatform.spec.defaults` (or the
...
**Acceptance Criteria:**
1. `KnativeAgent` SHALL be namespace-scoped.
...
2. WHEN `Canary` THE platform CR's `spec.canary.percent` selects
   what fraction of `KnativeAgent` CRs (by hash of name) actually
...
- One Go module shared with the rest of the repo
  (`github.com/stigen/knative-agents`); operator code lives under
  `operator/`.
