...
- [x] 1. Add Kubebuilder scaffolding under `operator/`
  ...
  - Action: `kubebuilder init --domain stigen.ai --repo
    github.com/stigen/knative-agents/operator --multigroup=false`
  - _Requirements: R-OP-API-1, R-OP-API-2_

- [x] 2. Generate API skeletons for `KnativeAgent` and
  `KnativeAgentPlatform`
  - Files: `operator/api/v1/knativeagent_types.go`,
    `operator/api/v1/knativeagentplatform_types.go`
  - Action: `kubebuilder create api --group agents --version v1 --kind KnativeAgent`
    then `--kind KnativeAgentPlatform --resource --controller`
  - _Requirements: R-OP-API-1, R-OP-API-2_
...
- [x] 6. Add printer columns + short names + categories
  - File: `operator/api/v1/knativeagent_types.go` annotations
...
- [x] 21. Implement `KnativeAgentReconciler` dispatching to features
  - File: `operator/internal/controllers/knativeagent_controller.go`
...
- [x] 23. Implement `KnativeAgentPlatformReconciler`
...
- [x] 37. Sample CRs
  - Files: `operator/config/samples/knativeagent_minimal.yaml`,
    `..._everything.yaml`
...
|------------------|----------------------------------------------------|------------------------------------------------------|
| R-OP-API-1       | operator/api/v1/knativeagent_types.go              | apply + kubectl get printer columns                  |
| R-OP-API-2       | operator/api/v1/knativeagentplatform_types.go      | webhook test: 2nd platform CR rejected               |
| R-OP-API-3       | kubebuilder markers                                | kubectl explain knativeagent.spec.features.identity  |
...
| R-OP-FF-4        | webhooks/knativeagent_webhook.go                   | webhook test: forbidden feature rejected             |
| R-OP-FF-5        | pkg/features/features.go                           | property test                                        |
| R-OP-REC-1       | controllers/knativeagent_controller.go             | envtest: deletion cascades                           |
...
| R-OP-WH-1        | webhooks/knativeagent_webhook.go                   | unit + envtest                                       |
| R-OP-WH-2        | webhooks/knativeagent_defaulter.go                 | unit                                                 |
| R-OP-WH-3        | webhooks/knativeagent_conversion.go                | property round-trip                                  |
...
| R-OP-ROLL-2      | controllers/knativeagent_controller.go             | envtest: paused = no apply                           |
