# Design — knative-agents-operator

## Overview

The operator is a Kubebuilder-built controller manager that watches two
custom resources (`KnativeAgent` namespaced and `KnativeAgentPlatform`
...
- **Existing Helm chart** — coexists for one minor version. Tenants
  migrate by `kubectl apply` of a `KnativeAgent` CR; the operator
...
```mermaid
graph TD
    User[kubectl apply\nKnativeAgent CR] --> Webhook[validating + mutating\nadmission webhooks]
    Webhook --> APIServer
...
### `operator/api/v1/knativeagent_types.go`
- **Purpose:** Public CR shape for tenants.
- **Highlights:**
  ```go
  type KnativeAgentSpec struct {
...
### `operator/internal/controllers/knativeagent_controller.go`
- **Purpose:** Top-level reconciler.
- **Reconcile flow:**
  1. Fetch CR; if not found, return.
  2. Resolve `KnativeAgentPlatform` defaults; merge into CR.
...
type FeatureReconciler interface {
    ...
    Reconcile(ctx context.Context,
              cr *v1.KnativeAgent,
              platform *v1.KnativeAgentPlatform) (FeatureResult, error)
}
...
- **Validating webhook** (`knativeagent_webhook.go`):
  - Enforces R-OP-WH-1 rules (insecure mode, runc gate, policy gate).
- **Mutating webhook** (`knativeagent_defaulter.go`):
  - Fills in defaults from `KnativeAgentPlatform`.
- **Conversion webhook** (`knativeagent_conversion.go`):
...
### `KnativeAgentStatus`
```go
type KnativeAgentStatus struct {
...
### Error Scenarios
1. **`KnativeAgentPlatform` missing** — every CR stuck in
...
- `make test-e2e-operator`:
  ...
  3. Apply `samples/knativeagent-everything.yaml`.
  4. `kubectl wait --for=condition=Ready knativeagent/sample`.
  5. Run a feature toggle script: turn each feature off then on,
     re-assert.
  6. Verify `kubectl get knativeagent -o wide` columns.
