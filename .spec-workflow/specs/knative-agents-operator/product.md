# Product Overview — knative-agents-operator

## Product Purpose

`knative-agents-operator` is a Kubebuilder-built controller manager that
...
sandbox, eBPF, secrets, Knative — as fields on a `KnativeAgent` custom
...
1. **`KnativeAgent` namespaced CR** — describes a single agent
   workload. Fields mirror `values.yaml` but are typed and validated.
2. **`KnativeAgentPlatform` cluster CR** — describes cluster-wide
   defaults (RuntimeClass, ebpf-loader DaemonSet, default trust
   domain) that one or more `KnativeAgent` CRs inherit.
...
4. **Feature gates with safe-by-default semantics** — a feature is
   ...
   degrading.
5. **Progressive rollout primitives** — each `KnativeAgent` carries a
...
7. **Validating + mutating admission webhooks** — block insecure
   ...
   `KNATIVE_AGENTS_ALLOW_INSECURE=true` annotation) at submit time
...
- **Make GitOps-native deployments first-class**: ArgoCD / Flux can
  reconcile against `KnativeAgent` CRs without dealing with chart
  values templating.
...
- **Reconcile p95**: ≤ 500 ms for a no-op reconcile (steady state)
  on a cluster with 200 `KnativeAgent` CRs.
...
- **`kubectl get knativeagents -A -o wide`** — printer columns:
  `MODE`, `READY`, `IDENTITY`, `MTLS`, `EBPF`, `SECRETS`,
  `RUNTIMECLASS`, `AGE`.
- **`kubectl describe knativeagent <name>`** — per-feature
...
- **`KnativeAgentSet`** (analogous to `Deployment` over `Pod`) for
