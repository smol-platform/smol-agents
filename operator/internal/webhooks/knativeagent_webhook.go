...
// `KNATIVE_AGENTS_ALLOW_INSECURE` runtime guard.
const AllowInsecureAnnotation = "knative-agents.stigen.ai/allow-insecure"
...
func ValidateAgent(cr *v1.KnativeAgent, platform *v1.KnativeAgentPlatform) error {
	var errs []error
...
func validatePolicy(cr *v1.KnativeAgent, platform *v1.KnativeAgentPlatform) []error {
	var errs []error
	...
	for _, row := range policy {
		...
		if enabledByName[row.Feature] {
			errs = append(errs, fmt.Errorf("feature %q is forbidden by KnativeAgentPlatform.spec.featurePolicy", row.Feature))
		}
	}
	return errs
}
...
func DefaultAgent(cr *v1.KnativeAgent, platform *v1.KnativeAgentPlatform) {
...
func ValidatePlatformSingleton(p *v1.KnativeAgentPlatform, singletonName string) error {
	if p.Name != singletonName {
		return fmt.Errorf("KnativeAgentPlatform name=%q is not %q (R-OP-API-2)", p.Name, singletonName)
	}
