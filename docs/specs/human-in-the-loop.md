...
> **✅ Decisions resolved 2026-06-03 — see [decisions.md](../design/decisions.md).** D6: **DEFERRED post-GA.** Ship only the cheap **harness pre-run approval gate** now; loop-mode mid-run continuation + resume (continuation-pods / step-wise contract) are post-GA. Where this doc still says OPEN/PROPOSED and conflicts, the decision log wins.
